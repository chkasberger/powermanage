...
public class FCGIOutputStream extends OutputStream {
	 ...
	 */
	public FCGIOutputStream(FileOutputStream outStream, int bufLen,
			int streamType, FCGIRequest inreq) {
		out = outStream;
		...
		if (!rawWrite) {
			...
			if (cLen > 0) {
				System.arraycopy(new FCGIMessage().makeHeader(type,
						request.requestID, cLen, 0), 0, buff, 0,
	...
	public void writeCloseRecords() throws IOException {
		 ...
		 */
		if (!(type == FCGIGlobalDefs.def_FCGIStderr && wrNext == 0 && !isAnythingWritten)) {
			byte hdr[] = new byte[FCGIGlobalDefs.def_FCGIHeaderLen];
			System.arraycopy(msg.makeHeader(type, request.requestID, 0, 0), 0,
					hdr, 0, FCGIGlobalDefs.def_FCGIHeaderLen);
		...
		if (request.numWriters == 1) {
			byte endReq[] = new byte[FCGIGlobalDefs.def_FCGIHeaderLen
					+ FCGIGlobalDefs.def_FCGIEndReqBodyLen];
			System.arraycopy(
					msg.makeHeader(FCGIGlobalDefs.def_FCGIEndRequest,
							request.requestID,
							FCGIGlobalDefs.def_FCGIEndReqBodyLen, 0), 0,
					endReq, 0, FCGIGlobalDefs.def_FCGIHeaderLen);
			System.arraycopy(msg.makeEndrequestBody(request.appStatus,
					FCGIGlobalDefs.def_FCGIRequestComplete), 0, endReq,
					FCGIGlobalDefs.def_FCGIHeaderLen,
					FCGIGlobalDefs.def_FCGIEndReqBodyLen);
